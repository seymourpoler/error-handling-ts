...
describe('Either should', () => {
    ...
    describe('Either left', () =>{
        test('create left either', () =>{
            const left = Either.createLeft(AppError.CannotSaveUser);
        ...
        function createError(appError: AppError) : CurrentError {
            return new CurrentError(appError);
        }
    
        test('fold left', () => {
            const appError = AppError.CannotSaveUser;
